...
def train_causal_epoch(model, optimizer, loader, device, args):
    ...
    for it, data in enumerate(loader):
        ...
        # o_loss = F.nll_loss(o_logs, one_hot_target) #for classification
        o_loss = F.l1_loss(o_logs, data.y.view(-1).float()) #for regression
        print('o_loss goood!!!!!!!!!!!!!!!!!!!!!!!')
        co_loss = model.mix_loss() #for regression
        print('co_loss good!!!!!!!!!!!!!!!!!')
        # co_loss = F.l1_loss(co_logs, data.y.view(-1).float()) #for regression
        # c_logs = F.log_softmax(c_logs, dim=-1)
        c_loss = torch.var(c_logs)
        # c_loss = F.l1_loss(c_logs, data.y.view(-1).float())
        # c_loss = F.kl_div(c_logs, uniform_target, reduction='batchmean')
        ...
        pred_o = o_logs.detach().view(-1)  # for regression
        true_o = data.y.view(-1).detach()  # 真实值展平成一维向量
        rmse += torch.sqrt(F.mse_loss(pred_o, true_o)).item()
    ...
    rmse = rmse / num
    return total_loss, total_loss_c, total_loss_o, total_loss_co, rmse
