...
def train_epoch(model, optimizer, loader, device, memory_bank,
    ...
    for it, data in enumerate(loader):
        ...
        data = data.to(device)
        out = model(data)
        loss = F.mse_loss(out, data.y.view(-1).float())
        loss.backward()
...
def train_causal_epoch(model, optimizer, loader, device, memory_bank,
    ...
    for step, data in enumerate(loader):
        ...
        if data.x.shape[0] == 1:
            ...
            o_loss = F.mse_loss(o_logs, data.y.view(-1).float()) #for regression
            # co_loss = F.nll_loss(co_logs, target_rep)
            co_loss = F.mse_loss(co_logs, data.y.view(-1).float()) #for regression
            c_logs = F.log_softmax(c_logs, dim=-1)
            c_loss = F.kl_div(c_logs, uniform_target, reduction='batchmean')
            # print('kl_loss',c_loss)
...
def loss_prototype(prototype, class_causal, criterion, args):
    ...
    labels = torch.zeros(distance.shape[0], dtype=torch.long).to(device)
    if len(class_causal) == 1:
