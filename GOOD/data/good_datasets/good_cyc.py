...
def make_mol(s: str, keep_h=False, add_h=False, keep_atom_map=False):
    """
...
class DomainGetter():
    ...
    def get_scaffold(self, mol) -> str:
        ...
        """
        try:
            scaffold = MurckoScaffold.MurckoScaffoldSmiles(mol=make_mol(mol), includeChirality=False)
            return scaffold
        except ValueError as e:
            print('Get scaffold error.')
            raise e

    def get_nodesize(self, mol) -> int:
        """
        Args:
            smile (str): A smile string for a molecule.
        Returns:
        ...
        #     return len(smile)
        number_atom = make_mol(mol).GetNumAtoms()
        return number_atom
...
class GOODCYC(InMemoryDataset):
    r"""
    The GOOD-CYC dataset. Adapted from 
    `Reaction profiles for quantum chemistry-computed [3+ 2] cycloaddition reactions.`_.
    ...
    def get_domain_sorted_list(self, data_list, domain='scaffold'):
        ...
        for data in tqdm(data_list):
            mol = data.mol
            data.__setattr__(domain, getattr(domain_getter, f'get_{domain}')(mol))
    ...
    def process(self):
        dataset = torch.load(f"../dataset/ReactionOOD/barriers_cycloadd.pt")[0].mol_graphs
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False):
        ...
        # print(train_dataset.num_node_features,train_dataset.num_edge_features)
